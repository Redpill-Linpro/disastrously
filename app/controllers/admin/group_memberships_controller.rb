...
class Admin::GroupMembershipsController < Admin::BaseController
  active_scaffold :group_membership do |config|

    config.columns[:parent].label = "Member of"

    config.actions = %w{list search}

    config.list.columns = %w{group parent updated_at created_at}

    config.list.sorting = [ { :group => :asc }, { :parent => :asc } ]
  end
end